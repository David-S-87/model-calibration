# Heston/evaluate_stage2.py

"""
Evaluate the Stage 2 posterior update network for the Heston model.
Compares prior vs true and posterior vs true for [mu, v0, theta, kappa, sigma_v, rho].
"""
...
CHECKPOINT     = os.path.join(CHECKPOINT_DIR, "stage2_heston_model.pth")
PLOTS_DIR      = os.path.join(os.path.dirname(__file__), "plots")
DEVICE         = torch.device("cuda" if torch.cuda.is_available() else "cpu")

PARAM_NAMES = ["mu", "v0", "theta", "kappa", "sigma_v", "rho"]
...
def main():
    ...
    for i, name in enumerate(PARAM_NAMES):
        print(f"{name:>7}     {prior_mse[i]:.6f}       {post_mse[i]:.6f}")
    ...
    for i, name in enumerate(PARAM_NAMES):
        ...
        plt.ylabel("Estimate")
        plt.title(f"Heston Stage 2: {name}")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        path = os.path.join(PLOTS_DIR, f"{name}_stage2_heston.png")
        plt.savefig(path)
